...
class ObjectInitializer(Grid):
    ...
    def populate_objects(self):
        ...
        for appt in self.schedule.appts:
            new_count = counts.get(appt.idnum, 0) + 1
            if new_count > 1:
                raise SystemExit(f'You cannot duplicate appointment id numbers! We found a duplicate: {appt.idnum}')
            else:
                counts[appt.idnum] = new_count
